...
|--------|-------------|---------|-----------|
| `MYSQL_HOST` | Sim | IP privado do servidor onde o MySQL está |
...
| `REDIS_HOST` | Sim | IP do Redis |
| `REDIS_PORT` | Sim | Porta do Redis |
| `API_KEY` | Não | Chave para o Lambda (vazio se não usar) |
| `DEPLOY_HOST` | Sim | IP ou hostname do servidor Ubuntu |
| `DEPLOY_USER` | Sim | Usuário SSH |
| `DEPLOY_SSH_KEY` | Sim | Chave privada SSH completa |
| `DEPLOY_PATH` | Sim | Caminho do projeto no servidor |
...
```sql
CREATE USER 'seu_usuario_aqui'@'%' IDENTIFIED BY 'sua_senha_segura';
GRANT ALL PRIVILEGES ON ai_agent_miraflores.* TO 'ai_agent'@'%';
FLUSH PRIVILEGES;
```

Use `seu_usuario_aqui` e a senha nos secrets `MYSQL_USER` e `MYSQL_PASSWORD`.
...
```bash
mkdir -p /home/diretorio_do/Ai-Agent
...
```bash
sudo cp /home/diretorio_do/Ai-Agent/deploy/ai-agent-api.service /etc/systemd/system/
sudo cp /home/diretorio_do/Ai-Agent/deploy/ai-agent-worker.service /etc/systemd/system/
...
|-------------|----------------|
| `seu_usuario` | Seu usuário Linux (ex: `seu_user_ai_agent`) |
| `/caminho/para/Ai-Agent` | Caminho real (ex: `/home/diretorio_do/Ai-Agent`) |
...
Type=simple
User=diretorio_do
WorkingDirectory=/home/diretorio_do/Ai-Agent
Environment="PATH=/home/diretorio_do/Ai-Agent/venv/bin"
ExecStart=/home/diretorio_do/Ai-Agent/venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 8000
Restart=always
RestartSec=5
